...
Purpose: Trace ingestion, schema normalization, and redacted trace storage.
...
MODULE_PURPOSE = 'Trace ingestion, schema normalization, and redacted trace storage.'
...
def load_trace_store(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = normalize_trace(materialized)
...
def trace_summary(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
