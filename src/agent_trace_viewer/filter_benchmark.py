...
"""Project-specific research helpers for Agent Trace Viewer.

Purpose: Search/filter latency benchmark helpers for static dashboards.
...
MODULE_PURPOSE = 'Search/filter latency benchmark helpers for static dashboards.'
...
def filter_traces(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def benchmark_filter(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = filter_traces(materialized)
...
def sample_queries(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
