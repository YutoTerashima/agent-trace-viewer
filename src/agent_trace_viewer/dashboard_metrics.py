...
Purpose: Risk, policy, latency, and tool-call aggregate metrics.
...
MODULE_PURPOSE = 'Risk, policy, latency, and tool-call aggregate metrics.'
...
def risk_distribution(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def latency_by_decision(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = risk_distribution(materialized)
...
def tool_risk_counts(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
