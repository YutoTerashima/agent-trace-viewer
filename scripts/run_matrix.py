...
def load_trace_source_rows(cfg: dict[str, Any], profile: str) -> list[dict[str, Any]]:
    limit = cfg["max_rows_smoke"] if profile == "smoke" else cfg["max_rows_full"]
    rows = read_jsonl(REPO / cfg["dataset_path"], limit=limit)
    rows = [{"text": r.get("text", ""), "label": r.get("label", "0"), "source_repo": "agent-trace-viewer-local"} for r in rows]
    for repo_name in ["agent-safety-eval-lab", "mcp-tool-security-playground"]:
        failure_path = REPO.parent / repo_name / "reports" / "results" / "v2_failure_cases.json"
        ...
        try:
            ...
            continue
        for row in failures[: min(1500, len(failures))]:
            true_label = str(row.get("true", "")).lower()
            pred_label = str(row.get("pred", "")).lower()
            label = "unsafe" if true_label == "unsafe" else "safe"
            rows.append({
                "text": f"{row.get('experiment_id', repo_name)} {row.get('failure_type', 'case')} score={row.get('score', '')} {row.get('text', '')}",
                "label": label,
                "source_repo": repo_name,
                "classifier_score": row.get("score"),
    ...
    if rows and len(rows) < limit:
        ...
        while len(rows) < limit:
            ...
            item["text"] = f"replay-{idx // len(base):03d} {item.get('text', '')}"
            item["source_repo"] = f"{item.get('source_repo', 'local')}:deterministic-replay"
            rows.append(item)
            idx += 1
    return rows[:limit]
...
def run_trace(cfg: dict[str, Any], profile: str, device: str) -> dict[str, Any]:
    rows = load_trace_source_rows(cfg, profile)
    traces, metrics = [], []
    for i, row in enumerate(rows):
        ...
        trace = {"trace_id": f"trace-{i:05d}", "source_repo": row.get("source_repo", "local"), "classifier_score": row.get("classifier_score"), "prompt": failure_text_for_release(text, label, decision), "label": label, "risk_score": risk, "policy_decision": decision, "latency_ms": latency, "tool_calls": [{"tool": "kb.search" if decision != "deny" else "file.read", "risk": "low" if decision == "allow" else "high"}], "failure_reason": row.get("failure_type") if row.get("failure_type") else (None if decision != "allow" or label not in {"1", "unsafe", "2"} else "unsafe_allowed")}
        traces.append(trace)
